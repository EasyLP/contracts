...
contract LiquidityOptimization is IERC721Receiver {
    ...
    external returns (
        ...
        TransferHelper.safeApprove(DAI, address(positionManager), amount0ToMint);
        TransferHelper.safeApprove(USDC, address(positionManager), amount1ToMint);
        ...
        if (amount0 < amount0ToMint) {
            TransferHelper.safeApprove(DAI, address(positionManager), 0);
            uint256 refund0 = amount0ToMint - amount0;
            TransferHelper.safeTransfer(DAI, msg.sender, refund0);
        }

        if (amount1 < amount1ToMint) {
            TransferHelper.safeApprove(USDC, address(positionManager), 0);
